...
export default function JsonToExcel({ title: toolTitle, description: toolDesc, lang }: { title?: string; description?: string; lang?: string } = {}) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">{toolTitle ?? 'JSON to Excel'}</h1>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">{toolDesc ?? 'Export JSON data to Excel (.xlsx) file.'}</p>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={handleExecute} className="rounded-lg bg-[var(--accent-blue)] px-6 py-2 text-sm font-medium text-white hover:opacity-90">{t.previewLabel}</button>
        {preview.length > 0 && (
          <button onClick={handleDownload} className="rounded-lg bg-[var(--accent-green)] px-6 py-2 text-sm font-medium text-white hover:opacity-90">{t.downloadExcel}</button>
        )}
        ...
        <button onClick={() => { setInput(''); setPreview([]); setError(''); }} className="rounded-lg border border-[var(--border-primary)] px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]">{t.clear}</button>
      </div>
