...
export default function JsonCompare({ title: toolTitle, description: toolDesc, lang }: { title?: string; description?: string; lang?: string } = {}) {
  ...
  function handleCompare() {
    setError('');
    setDiffs([]);
    if (!left.trim() || !right.trim()) { setError(t.pleaseEnterBoth); return; }
    try {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">{toolTitle ?? 'JSON Compare'}</h1>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">{toolDesc ?? 'Compare two JSON objects and highlight differences.'}</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">{t.originalJson}</label>
          <textarea
            value={left}
            onChange={(e) => setLeft(e.target.value)}
            placeholder={t.phOriginalJson}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">{t.modifiedJson}</label>
          <textarea
            value={right}
            onChange={(e) => setRight(e.target.value)}
            placeholder={t.phModifiedJson}
            className="w-full rounded-lg border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-3 font-mono text-sm text-[var(--text-primary)] outline-none focus:border-[var(--accent-blue)] h-[250px] resize-none"
          />
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={handleCompare} className="rounded-lg bg-[var(--accent-blue)] px-6 py-2 text-sm font-medium text-white hover:opacity-90">{t.compare}</button>
        <button onClick={() => { setLeft(EXAMPLE_LEFT); setRight(EXAMPLE_RIGHT); }} className="rounded-lg border border-[var(--border-primary)] px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]">{t.example}</button>
        <button onClick={() => { setLeft(''); setRight(''); setDiffs([]); setError(''); }} className="rounded-lg border border-[var(--border-primary)] px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]">{t.clear}</button>
      </div>
      ...
      {diffs.length > 0 && (
        <div className="rounded-lg border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-4">
          <h3 className="mb-3 text-sm font-semibold text-[var(--text-primary)]">
            {t.differencesFound} ({diffs.length})
          </h3>
