...
export default function QrCodeDecoder({ title: toolTitle, description: toolDesc, lang }: { title?: string; description?: string; lang?: string } = {}) {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    img.onload = () => {
      ...
      if (!ctx) { setError(t.canvasNotSupported); return; }
    ...
    img.onerror = () => { URL.revokeObjectURL(objectUrl); setError(t.failedLoadImage); };
    img.src = objectUrl;
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">{toolTitle ?? 'QR Code Decoder'}</h1>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">{toolDesc ?? 'Upload an image to decode the QR code embedded in it.'}</p>
      </div>
      ...
      {result && (
        <div className="rounded-lg border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-4">
          <h3 className="mb-2 text-sm font-medium text-[var(--text-secondary)]">{t.decodedContent}</h3>
