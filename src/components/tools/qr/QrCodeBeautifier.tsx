...
export default function QrCodeBeautifier() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">QR Code Beautifier</h1>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">Generate styled QR codes with custom colors and size.</p>
      </div>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Content</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Foreground</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Size: {size}px</label>
              <input type="range" min={100} max={1000} step={50} value={size} onChange={(e) => setSize(Number(e.target.value))} className="w-full" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Margin: {margin}</label>
              <input type="range" min={0} max={10} value={margin} onChange={(e) => setMargin(Number(e.target.value))} className="w-full" />
            </div>
          </div>
          <button onClick={handleGenerate} className="w-full rounded-lg bg-[var(--accent-blue)] py-2 text-sm font-medium text-white hover:opacity-90">Generate QR Code</button>
        </div>
          ...
          {dataUrl ? (
            <>
              ...
              <button onClick={() => { const a = document.createElement('a'); a.href = dataUrl; a.download = `qr-beautified-${Date.now()}.png`; a.click(); }}
                className="mt-4 rounded-lg border border-[var(--border-primary)] px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]">Download PNG</button>
            </>
          ) : (
            <span className="text-sm text-[var(--text-tertiary)]">QR code preview</span>
          )}
        </div>
      </div>
